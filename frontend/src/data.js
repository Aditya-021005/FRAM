...
const generateStockData = (ticker, isLiquidGroup = true) => {
  ...
  // Ground truth for selected pair
  ...
  const avgAmihud = isLiquidGroup ? 0.0004 + (seed % 10)/100000 : 0.005 + (rank / 1000);
  const avgRollingVol = isLiquidGroup ? 17 + (seed % 5) : 15 + (seed % 4);

  return {
    ticker, rank,
    stats: { meanReturn, stdReturn, avgAmihud, avgRollingVol, avgTurnover: stockInfo.turnover },
    vol: { histVol: avgRollingVol, garchVol: avgRollingVol * 1.04, longRunVol: avgRollingVol * 0.94, persistence: 0.93 + (seed % 4)/100 },
    correlation: { vol_amihud: 0.12 + (seed % 15)/100 },
    options: [
      { strikeLabel: "ATM", optType: "Call", mktPrice: 40 + (seed % 30), bsmHist: 38 + (seed % 25), bsmGarch: 42 + (seed % 28), delta: 0.48, gamma: 0.002, vega: 1.1, dte: 29 },
      { strikeLabel: "OTM_Call", optType: "Call", mktPrice: 12 + (seed % 5), bsmHist: 11 + (seed % 4), bsmGarch: 13 + (seed % 6), delta: 0.14, gamma: 0.001, vega: 0.4, dte: 29 },
      { strikeLabel: "OTM_Put", optType: "Put", mktPrice: 14 + (seed % 6), bsmHist: 13 + (seed % 5), bsmGarch: 15 + (seed % 7), delta: -0.16, gamma: 0.001, vega: 0.5, dte: 29 }
    ],
    pnl: [{ priceShock: "-2%", volShock: "-20%", totalPnl: -300 - (seed % 100) }, { priceShock: "+2%", volShock: "+20%", totalPnl: 400 + (seed % 100) }],
    var: [
...
export const getIlliquidData = (ticker) => generateStockData(ticker, ticker === "HDFCBANK.NS");
